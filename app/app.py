    ...
    if st.session_state.analysis_cache is not None:
        ...
        if st.session_state.get("show_report_modal", False):
            ...
            for i, (c_name, prob) in enumerate(top_k):
                ...
                bars_html += f"""<div style="background: #F8FAFC; border: 1.5px solid #E2E8F0; border-radius: 12px; padding: 10px 16px; margin-bottom: 10px; position: relative; overflow: hidden;"><div style="position: absolute; left: 0; top: 0; bottom: 0; width: {prob:.1f}%; background: {bar_bg}; opacity: {'1.0' if i == 0 else '0.4'}; border-radius: 10px;"></div><div style="position: relative; z-index: 2; display: flex; justify-content: space-between; align-items: center;"><span style="font-size: 1.05rem; font-weight: 800; color: {text_col};">#{i+1} &nbsp;{c_name}</span><span style="font-size: 1.05rem; font-weight: 900; color: {'#FFFFFF' if (i == 0 and prob > 80) else val_col};">{prob:.2f}%</span></div></div>"""

            # Crystal-Clear, Razor-Sharp, Luxury Interactive Certificate Preview
            cert_full_html = f"""<div style="background: #FFFFFF; border: 3.5px solid #0F172A; border-radius: 24px; padding: 24px; box-shadow: 0 16px 40px rgba(15, 23, 42, 0.14); font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #0F172A; margin-bottom: 20px;">
<div style="border: 2px solid #0284C7; border-radius: 16px; padding: 20px; background: #FFFFFF;">
<div style="background: #F8FAFC; border: 1.5px solid #E2E8F0; border-radius: 14px; padding: 18px 20px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 14px;">
<div>
<div style="display: inline-flex; align-items: center; gap: 6px; background: #E0F2FE; color: #0369A1; font-size: 0.82rem; font-weight: 900; padding: 4px 12px; border-radius: 999px; text-transform: uppercase; margin-bottom: 6px; border: 1px solid #0284C7;">
...
<div style="font-size: 0.92rem; font-weight: 700; color: #475569; margin-top: 4px;">
PyTorch ResNet-18 Deep Transfer Learning Architecture • Native Grad-CAM XAI
</div>
</div>
<div style="display: flex; flex-direction: column; align-items: flex-end; gap: 6px;">
<div style="font-size: 0.82rem; font-weight: 800; font-family: monospace; color: #0284C7; background: #F0F9FF; padding: 3px 8px; border-radius: 6px; border: 1px solid #BAE6FD;">
CERT-ID: BIO-2026-XAI-{cert_hash}
</div>
<div style="background: #ECFDF5; border: 2px solid #10B981; border-radius: 12px; padding: 6px 14px; text-align: right;">
<div style="font-size: 0.72rem; font-weight: 900; color: #059669; text-transform: uppercase;">VERIFIED AI DECISION</div>
<div style="font-size: 1.45rem; font-weight: 900; color: #065F46;">CONFIDENCE: {confidence:.1f}%</div>
</div>
</div>
</div>
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 20px;">
<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
<div style="background: #FFFFFF; border: 1.5px solid #CBD5E1; border-radius: 14px; padding: 8px; text-align: center;">
<img src="data:image/png;base64,{orig_b64}" style="width: 100%; aspect-ratio: 1/1; object-fit: contain; border-radius: 8px; display: block; margin-bottom: 6px;" />
<div style="background: #F1F5F9; border: 1px solid #CBD5E1; border-radius: 6px; padding: 5px 2px; font-size: 0.84rem; font-weight: 800; color: #0F172A;">
Original Input Specimen
</div>
</div>
<div style="background: #FFFFFF; border: 2px solid #0284C7; border-radius: 14px; padding: 8px; text-align: center;">
<img src="data:image/png;base64,{overlay_b64}" style="width: 100%; aspect-ratio: 1/1; object-fit: contain; border-radius: 8px; display: block; margin-bottom: 6px;" />
<div style="background: #E0F2FE; border: 1px solid #0284C7; border-radius: 6px; padding: 5px 2px; font-size: 0.84rem; font-weight: 800; color: #0369A1;">
...
<div>
<div style="display: inline-block; background: #E0F2FE; color: #0369A1; font-size: 0.78rem; font-weight: 900; padding: 3px 10px; border-radius: 6px; border: 1px solid #0284C7; margin-bottom: 6px; text-transform: uppercase;">
TAXONOMIC CLASSIFICATION
</div>
<div style="font-size: 2.1rem; font-weight: 900; color: #0F172A; line-height: 1.1; margin-bottom: 4px;">
{pred_class}
</div>
<div style="font-size: 1.15rem; font-style: italic; font-weight: 800; color: #0284C7; margin-bottom: 4px;">
Scientific Name: {report_meta['scientific_name']}
</div>
<div style="font-size: 0.98rem; font-weight: 700; color: #475569; margin-bottom: 12px;">
Taxonomic Family: {report_meta['family']}
</div>
</div>
<div>
<div style="font-size: 0.98rem; font-weight: 900; color: #0F172A; margin-bottom: 8px;">
...
<div style="background: #FFFFFF; border: 2px solid #0284C7; border-radius: 16px; overflow: hidden;">
<div style="background: #F0F9FF; border-bottom: 1.5px solid #BAE6FD; padding: 10px 16px; font-size: 1.0rem; font-weight: 900; color: #0369A1;">
🔬 NEURO-VISUAL ATTENTION (GRAD-CAM XAI)
</div>
<div style="padding: 16px;">
<div style="font-size: 0.95rem; font-weight: 900; color: #0F172A; margin-bottom: 4px;">
• Gradient Hotspot Diagnostic:
</div>
<div style="font-size: 0.92rem; color: #1E293B; line-height: 1.5; font-weight: 600; margin-bottom: 12px;">
...
<div style="font-size: 0.88rem; color: #64748B; font-weight: 700;">
PyTorch ResNet-18 (512-dim bottleneck) + Target Layer-4 Feature Maps
</div>
</div>
</div>
<div style="background: #FFFFFF; border: 2px solid #10B981; border-radius: 16px; overflow: hidden;">
<div style="background: #F0FDF4; border-bottom: 1.5px solid #BBF7D0; padding: 10px 16px; font-size: 1.0rem; font-weight: 900; color: #065F46;">
...
<div style="font-size: 0.88rem; color: #059669; font-weight: 800;">
Validated by Cross-Entropy Loss Optimization & Grad-CAM Backprop
</div>
</div>
</div>
</div>
<div style="border-top: 1.5px solid #E2E8F0; padding-top: 14px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
<div>
...
<div style="font-size: 0.82rem; font-weight: 600; color: #64748B; margin-top: 2px;">
TorchScript Mobile Export Ready • Native Explainable AI Grad-CAM Studio
</div>
</div>
<div style="text-align: right;">
<div style="font-size: 0.94rem; font-weight: 900; color: #0284C7;">
